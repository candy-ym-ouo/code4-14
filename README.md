...
- 项目、材料需求、计划与实际消耗对比。
- 消耗、损耗、撤销和反向库存流水。
- 项目用料成本核算：按批次、用量和币种分摊采购费与损耗，多币种折算本位币。
- 汇率与采购费用补录，成本重算稳定且历史凭证不可改写。
- 颜色变化时间线和批次当前颜色。
...
- 金额使用 `numeric(18,2)`，汇率使用 `numeric(18,8)`，成本运算全程定点数。
- 批次成本 = 货款 × 入库日汇率 + 采购费 × 各自发生日汇率，按消耗数量占比分摊。
- 成本重算结果不变时不产生新记录；变化时生成新凭证并显式作废旧凭证。
- 汇率、采购费用、成本凭证均为 append-only，数据库触发器拒绝修改和删除。
- 归档代替核心数据硬删除。
- 审计日志只追加，不更新、不删除。
